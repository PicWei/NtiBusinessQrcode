package com.nti.module_sortingoutbound.repository;
 ...
 * @describe
 */
public class SortingoutboundRepo {
    public MutableLiveData<List<SortingoutboundOrderInfo>> PDA_H(Paramer paramer){
        final MutableLiveData<List<SortingoutboundOrderInfo>> data = new MutableLiveData<>();
        final List<SortingoutboundOrderInfo> orderInfos = new ArrayList<>();
        HttpUtils.getInstance().with(IService.class).PDA_H(paramer)
                ...
                .subscribe(new Observer<JsonObject>() {
                    ...
                    public void onNext(@NotNull JsonObject jsonObject) {
                        ...
                        List<SortingoutboundDetail> salesFactoryDetails = new ArrayList<>();
                        try {
                            for (int i = 0; i < jsonArray1.size(); i++){
                                ...
                                List<SortingoutboundBarcode> barcodes = LitePal.where("UUID = ? and pcigcode = ? and isSubmit = 0", uuid, pickCode).find(SortingoutboundBarcode.class);
                                ...
                                SortingoutboundDetail salesFactoryDetail = new SortingoutboundDetail(pickName, billPNum, billBNum, uuid, pickCode, scanBNum, String.valueOf(mscanQty));
                        ...
                        try {
                            ...
                            /*if (object.has("BB_TOTAL_SCAN_NUM")){
                                ...
                                List<SortingoutboundDetail> details = LitePal.where("BD_BB_UUID = ?", BB_UUID).find(SortingoutboundDetail.class);
                                ...
                                SortingoutboundOrderInfo orderInfo = new SortingoutboundOrderInfo(BB_UUID, BB_CONTRACT_NO, BB_BT_CODE, BB_TICKET_NO, BB_WS_CODE,
                                        BB_RELATE_CONTRACT_NO, B_NAME, BB_INPUT_DATE, BB_TOTAL_ALL_NUM1, BB_TOTAL_SCAN_NUM, BB_TOTAL_PNUM, BB_FLOW_NAME, BB_STATE);
                                orderInfos.add(orderInfo);
                            }
                            LitePal.deleteAll(SortingoutboundOrderInfo.class);
