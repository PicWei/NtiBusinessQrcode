...
public class SortingoutboundDetailActivity extends BaseActivity implements View.OnClickListener {
    ...
    public void onClick(View view) {
        ...
        if (view.getId() == R.id.confirm_brn){
            ...
            List<SortingoutboundBarcode> barcodes = LitePal.where("UUID = ? and isSubmit = 0", uuid).find(SortingoutboundBarcode.class);
            if (barcodes.size() == 0){
                                            ...
                                            viewModel.updataSellListStatues(upParamer).observe(SortingoutboundDetailActivity.this, new Observer<JsonObject>() {
                                                ...
                                                public void onChanged(JsonObject jsonObject) {
                                                    if (jsonObject == null){
                                                        ...
                                                        if (code.equals("0")){
                                                            SortingoutboundOrderInfo orderInfo = orderInfos.get(0);
        ...
        if (data != null) {
            ...
            if (bundle != null) {
                if (bundle.getInt(XQRCode.RESULT_TYPE) == XQRCode.RESULT_SUCCESS) {
                    ...
                    int count = 0;
                    List<SortingoutboundBarcode> barcodes = LitePal.where("barcode = ?", result).find(SortingoutboundBarcode.class);
                    if (barcodes.size() > 0){
                        playSound(2);
                    ...
                    for (int i = 0; i < detailList2.size(); i++){
                        ...
                        if (pcigCode.equals(code)){
                            ...
                            SortingoutboundBarcode salesBarcode = new SortingoutboundBarcode(uuid, pcigCode, pcigName, result, scantime, scancode);
                            salesBarcode.save();
