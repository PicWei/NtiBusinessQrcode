package com.nti.module_sortingoutbound.activity;
...
import com.nti.lib_common.bean.SortingoutboundBarcode;
...
import com.nti.module_sortingoutbound.adapter.SortingoutboundDetailAdapter;
import com.nti.module_sortingoutbound.databinding.ActivitySortingoutboundDetailBinding;
...
@Route(path = ARouterPath.SORTINGOUTBOUNDDETAIL_PATH)
public class SortingoutboundDetailActivity extends BaseActivity implements View.OnClickListener {

    private ActivitySortingoutboundDetailBinding binding;
    private SortingoutboundDetailAdapter adapter;

    private List<SortingoutboundDetail> detailList = new ArrayList<>();

    private List<SortingoutboundDetail> detailList2 = new ArrayList<>();
    ...
    public static final String SYSTEM_SERVICE_TYPE = "US_SORT_BILL_BASE";


    private SortingoutboundVM ViewModel;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        binding = DataBindingUtil.setContentView(this, R.layout.activity_sortingoutbound_detail);
        ...
        ViewModel = new ViewModelProvider(this).get(SortingoutboundVM.class);
    ...
    private void loadLatelyData() {
        SortingoutboundOrderInfo info = LitePal.where("BB_UUID = ?", uuid).findFirst(SortingoutboundOrderInfo.class);
        if (info!=null){
            A_NO = info.getA_NO();
        }
    }
    ...
    protected void onStart() {
        ...
        SortingoutboundOrderInfo orderInfo = LitePal.where("BB_UUID = ?", uuid).findFirst(SortingoutboundOrderInfo.class);
        ...
        detailList2 = LitePal.where("BD_BB_UUID = ?", uuid).find(SortingoutboundDetail.class);

        detailList.addAll(detailList2);
        adapter = new SortingoutboundDetailAdapter(detailList, this);
        binding.recyclerView.setAdapter(adapter);
        adapter.setOnItemClickListener(new SortingoutboundDetailAdapter.OnItemClickListener() {
            @Override
            public void onItemClick(SortingoutboundDetail detail) {
                String pcigcode = detail.getBD_PCIG_CODE();
                List<SortingoutboundBarcode> barcodes = LitePal.where("pcigcode = ? and isSubmit = 0", pcigcode).find(SortingoutboundBarcode.class);
                Collections.sort(barcodes, new Comparator<SortingoutboundBarcode>() {
                    @Override
                    public int compare(SortingoutboundBarcode salesBarcode, SortingoutboundBarcode t1) {
                        return t1.getScantime().compareTo(salesBarcode.getScantime());
                    }
                });
                BasePopupView popupView = new BarcodeListPopup(SortingoutboundDetailActivity.this);
                new XPopup.Builder(SortingoutboundDetailActivity.this)
    ...
    public void onClick(View view) {
        ...
        if (view.getId() == R.id.confirm_brn){
            ...
            int barcode_count = LitePal.where("UUID = ? and isSubmit = 0", uuid).count(SortingoutboundBarcode.class);
            if (barcode_count == 0){
                            ...
                            .asConfirm(null, "当前单据还未扫描完全，确认完成？",
                                    ...
                                    new OnConfirmListener() {
                                        ...
                                        public void onConfirm() {
                                            ...
                                            ViewModel.updataSellListStatues(upParamer).observe(SortingoutboundDetailActivity.this, new Observer<JsonObject>() {
                                                @Override
                                                public void onChanged(JsonObject jsonObject) {
                                                    if (jsonObject == null){
                                                        Toast.makeText(SortingoutboundDetailActivity.this, "网络异常", Toast.LENGTH_SHORT).show();
                                                    }else {
                                                        String code = jsonObject.get("code").toString().replace("\"", "");
                                                        if (code.equals("0")){
                                                            orderInfo.setBB_STATE("3");
                                                            orderInfo.saveOrUpdate("BB_UUID = ?", uuid);
                                                            Toast.makeText(SortingoutboundDetailActivity.this, "确认成功", Toast.LENGTH_SHORT).show();
                                                        }else {
                                                            Toast.makeText(SortingoutboundDetailActivity.this, "更改状态失败", Toast.LENGTH_SHORT).show();
                                                        }
                                                    }
                                                }
                                            });
    ...
    private void handleScannerResult(String result) {
        ...
        int count = 0;
        int size = LitePal.where("barcode = ?", result).count(SortingoutboundBarcode.class);
        ...
        for (int i = 0; i < detailList2.size(); i++) {
            ...
            SortingoutboundDetail detail = detailList2.get(i);
            String pcigCode = detail.getBD_PCIG_CODE();
            ...
            if (pcigcodesub.equals(code)) {
                ...
                SortingoutboundBarcode SortingoutboundBarcode = new SortingoutboundBarcode(uuid, pcigCode, pcigName, result, scantime, scancode);
                SortingoutboundBarcode.save();
                String new_scanQty = String.valueOf(mscanQty);
                ContentValues cv = new ContentValues();
                cv.put("BD_SCAN_NUM", new_scanQty);
                LitePal.updateAll(SortingoutboundDetail.class, cv, "BD_PCIG_CODE = ?", pcigCode);
                ContentValues cv2 = new ContentValues();
                List<SortingoutboundOrderInfo> orderInfos = LitePal.where("BB_UUID = ?", uuid).find(SortingoutboundOrderInfo.class);
                ...
                LitePal.updateAll(SortingoutboundOrderInfo.class, cv2, "BB_UUID = ?", uuid);
                ...
                sellBarcodeReciveViewModel.sellBarcodeRecive(paramer).observe(this, new Observer<DataResult<JsonObject>>() {
                    ...
                    public void onChanged(DataResult<JsonObject> dataResult) {
                        ...
                        if (errcode == 0) {
                            ...
                            SortingoutboundBarcode barcode = new SortingoutboundBarcode(uuid, pcigCode, picgname, result, scantime, scancode);
                            barcode.setSubmit(true);
                            barcode.save();
                            Toast.makeText(SortingoutboundDetailActivity.this, "sucess", Toast.LENGTH_SHORT).show();
        ...
        ViewModel.errorSignReceive(esrparamer).observe(this, new Observer<JsonObject>() {
            ...
            public void onChanged(JsonObject jsonObject) {
                if (jsonObject == null){
                    ...
                    Toast.makeText(SortingoutboundDetailActivity.this, "异常条码上传成功", Toast.LENGTH_SHORT).show();
                }
            }
        });
    }
