...
public class SortingoutboundActivity extends BaseActivity implements View.OnClickListener {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        viewModel.PDA_H(paramer).observe(this, new Observer<List<SortingoutboundOrderInfo>>() {
            @Override
            public void onChanged(List<SortingoutboundOrderInfo> sortingoutboundOrderInfos) {
                loadingPopup.dismiss();
                if (sortingoutboundOrderInfos == null){
                    ...
                    List<SortingoutboundOrderInfo> orderInfos2 = LitePal.where("BB_STATE = ?", "1").find(SortingoutboundOrderInfo.class);
                    List<SortingoutboundOrderInfo> orderInfos3 = LitePal.where("BB_STATE = ?", "3").find(SortingoutboundOrderInfo.class);
    ...
    public void onClick(View view) {
        if (view.getId() == R.id.incomplete_cl){
            ...
            viewModel.PDA_H(paramer).observe(this, new Observer<List<SortingoutboundOrderInfo>>() {
                @Override
                public void onChanged(List<SortingoutboundOrderInfo> salesFactoryOrderInfos) {
                    loadingPopup.dismiss();
    ...
    public void onMessageEvent(MessageEvent event){
        switch (event.what){
            case BusinessType.UPDATA_ONGOING:
                List<SortingoutboundOrderInfo> orderInfos2 = LitePal.where("BB_STATE = ? or BB_STATE = ? or BB_STATE = ?", "1", "3", "4").find(SortingoutboundOrderInfo.class);
                int ongoingNum = orderInfos2.size();
                binding.ongoingNum.setText(ongoingNum+"");
                break;
        }
