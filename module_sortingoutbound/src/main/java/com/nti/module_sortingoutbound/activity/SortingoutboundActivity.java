...
import com.nti.module_sortingoutbound.databinding.ActivitySortingoutboundBinding;
import com.nti.module_sortingoutbound.fragment.CompletedFragment;
import com.nti.module_sortingoutbound.fragment.IncompleteFragment;
import com.nti.module_sortingoutbound.fragment.OngoingFragment;
...
//商业分拣领用出库 US_SORT_BILL_BASE
@Route(path = ARouterPath.SORTINGOUTBOUND_PATH)
public class SortingoutboundActivity extends BaseActivity implements View.OnClickListener {

    private ActivitySortingoutboundBinding binding;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        binding = DataBindingUtil.setContentView(this, R.layout.activity_sortingoutbound);
    ...
    private void loadData() {
        ...
        viewModel.PDA_H(paramer).observe(this, new Observer<DataResult<List<SortingoutboundOrderInfo>>>() {
            @Override
            public void onChanged(DataResult<List<SortingoutboundOrderInfo>> dataResult) {
                loadingPopup.dismiss();
                int errcode = dataResult.getErrcode();
                if (errcode == -1){
                    Toast.makeText(SortingoutboundActivity.this, "网络异常", Toast.LENGTH_SHORT).show();
                }else {
                    List<SortingoutboundOrderInfo> SortingoutboundOrderInfos = dataResult.getT();
                    if (SortingoutboundOrderInfos == null || SortingoutboundOrderInfos.isEmpty()){
                        Toast.makeText(SortingoutboundActivity.this, "数据为空", Toast.LENGTH_SHORT).show();
                    }else {
                        binding.incompleteCl.performClick();
                        loadViewCount();
                    }
    ...
    private void loadViewCount() {
        unFinishCount = LitePal.where("BB_STATE = ?", "4").count(SortingoutboundOrderInfo.class);
        doingCount = LitePal.where("BB_STATE = ?", "1").count(SortingoutboundOrderInfo.class);
        finishCount = LitePal.where("BB_STATE = ? and PDA_SCANNER_IS_END = ?", "3", "0").count(SortingoutboundOrderInfo.class);
