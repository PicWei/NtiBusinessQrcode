package com.nti.module_arrivalinbound.activity;
...
import com.nti.lib_common.bean.ArrivalInboundBarcode;
...
import com.nti.module_arrivalinbound.adapter.ArrivalInboundDetailAdapter;
import com.nti.module_arrivalinbound.databinding.ActivityArrivalInboundDetailBinding;
import com.nti.module_arrivalinbound.viewmodel.ArrivalInboundViewModel;
...
@Route(path = ARouterPath.ARRIVALINBOUNDDETAIL_PATH)
public class ArrivalInboundDetailActivity extends BaseActivity implements View.OnClickListener{

    private ActivityArrivalInboundDetailBinding binding;
    private ArrivalInboundDetailAdapter adapter;

    private List<ArrivalInboundDetail> detailList = new ArrayList<>();

    private List<ArrivalInboundDetail> detailList2 = new ArrayList<>();
    ...
    private ArrivalInboundViewModel ViewModel;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        binding = DataBindingUtil.setContentView(this, R.layout.activity_arrival_inbound_detail);
        ...
        ViewModel = new ViewModelProvider(this).get(ArrivalInboundViewModel.class);
    ...
    private void loadLatelyData() {
        ArrivalInboundOrderInfo info = LitePal.where("BB_UUID = ?", uuid).findFirst(ArrivalInboundOrderInfo.class);
        if (info!=null){
            A_NO = info.getA_NO();
        }
    }
    ...
    protected void onStart() {
        ...
        ArrivalInboundOrderInfo orderInfo = LitePal.where("BB_UUID = ?", uuid).findFirst(ArrivalInboundOrderInfo.class);
        ...
        detailList2 = LitePal.where("BD_BB_UUID = ?", uuid).find(ArrivalInboundDetail.class);

        detailList.addAll(detailList2);
        adapter = new ArrivalInboundDetailAdapter(detailList, this);
        binding.recyclerView.setAdapter(adapter);
        adapter.setOnItemClickListener(new ArrivalInboundDetailAdapter.OnItemClickListener() {
            @Override
            public void onItemClick(ArrivalInboundDetail detail) {
                String pcigcode = detail.getBD_PCIG_CODE();
                List<ArrivalInboundBarcode> barcodes = LitePal.where("pcigcode = ? and isSubmit = 0", pcigcode).find(ArrivalInboundBarcode.class);
                Collections.sort(barcodes, new Comparator<ArrivalInboundBarcode>() {
                    @Override
                    public int compare(ArrivalInboundBarcode salesBarcode, ArrivalInboundBarcode t1) {
                        return t1.getScantime().compareTo(salesBarcode.getScantime());
                    }
                });
                BasePopupView popupView = new BarcodeListPopup(ArrivalInboundDetailActivity.this);
                new XPopup.Builder(ArrivalInboundDetailActivity.this)
    ...
    public void onClick(View view) {
        ...
        if (view.getId() == R.id.confirm_brn){
            ...
            int barcode_count = LitePal.where("UUID = ? and isSubmit = 0", uuid).count(ArrivalInboundBarcode.class);
            if (barcode_count == 0){
                            ...
                            .asConfirm(null, "当前单据还未扫描完全，确认完成？",
                                    ...
                                    new OnConfirmListener() {
                                        ...
                                        public void onConfirm() {
                                            ...
                                            ViewModel.updataSellListStatues(upParamer).observe(ArrivalInboundDetailActivity.this, new Observer<JsonObject>() {
                                                @Override
                                                public void onChanged(JsonObject jsonObject) {
                                                    if (jsonObject == null){
                                                        Toast.makeText(ArrivalInboundDetailActivity.this, "网络异常", Toast.LENGTH_SHORT).show();
                                                    }else {
                                                        String code = jsonObject.get("code").toString().replace("\"", "");
                                                        if (code.equals("0")){
                                                            orderInfo.setBB_STATE("3");
                                                            orderInfo.saveOrUpdate("BB_UUID = ?", uuid);
                                                            Toast.makeText(ArrivalInboundDetailActivity.this, "确认成功", Toast.LENGTH_SHORT).show();
                                                        }else {
                                                            Toast.makeText(ArrivalInboundDetailActivity.this, "更改状态失败", Toast.LENGTH_SHORT).show();
                                                        }
                                                    }
                                                }
                                            });
    ...
    private void handleScannerResult(String result) {
        ...
        int count = 0;
        int size = LitePal.where("barcode = ?", result).count(ArrivalInboundBarcode.class);
        ...
        for (int i = 0; i < detailList2.size(); i++) {
            ...
            ArrivalInboundDetail detail = detailList2.get(i);
            String pcigCode = detail.getBD_PCIG_CODE();
            ...
            if (pcigcodesub.equals(code)) {
                ...
                ArrivalInboundBarcode ArrivalInboundBarcode = new ArrivalInboundBarcode(uuid, pcigCode, pcigName, result, scantime, scancode);
                ArrivalInboundBarcode.save();
                String new_scanQty = String.valueOf(mscanQty);
                ContentValues cv = new ContentValues();
                cv.put("BD_SCAN_NUM", new_scanQty);
                LitePal.updateAll(ArrivalInboundDetail.class, cv, "BD_PCIG_CODE = ?", pcigCode);
                ContentValues cv2 = new ContentValues();
                List<ArrivalInboundOrderInfo> orderInfos = LitePal.where("BB_UUID = ?", uuid).find(ArrivalInboundOrderInfo.class);
                ...
                LitePal.updateAll(ArrivalInboundOrderInfo.class, cv2, "BB_UUID = ?", uuid);
                ...
                sellBarcodeReciveViewModel.sellBarcodeRecive(paramer).observe(this, new Observer<DataResult<JsonObject>>() {
                    ...
                    public void onChanged(DataResult<JsonObject> dataResult) {
                        ...
                        if (errcode == 0) {
                            ...
                            ArrivalInboundBarcode barcode = new ArrivalInboundBarcode(uuid, pcigCode, picgname, result, scantime, scancode);
                            barcode.setSubmit(true);
                            barcode.save();
                            Toast.makeText(ArrivalInboundDetailActivity.this, "sucess", Toast.LENGTH_SHORT).show();
        ...
        ViewModel.errorSignReceive(esrparamer).observe(this, new Observer<JsonObject>() {
            ...
            public void onChanged(JsonObject jsonObject) {
                if (jsonObject == null){
                    ...
                    Toast.makeText(ArrivalInboundDetailActivity.this, "异常条码上传成功", Toast.LENGTH_SHORT).show();
                }
            }
        });
    }
