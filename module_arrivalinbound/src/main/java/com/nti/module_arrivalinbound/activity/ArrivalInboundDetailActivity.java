...
public class ArrivalInboundDetailActivity extends BaseActivity implements View.OnClickListener{
    ...
    public void onClick(View view) {
        ...
        if (view.getId() == R.id.confirm_brn){
            ...
            List<ArrivalInboundBarcode> barcodes = LitePal.where("UUID = ? and isSubmit = 0", uuid).find(ArrivalInboundBarcode.class);
            if (barcodes.size() == 0){
                    ...
                    BasePopupView popupView = new XPopup.Builder(this)
                            ...
                            .asConfirm(null, "当前单据还未扫描完全，确认完成？",
                                    ...
                                    new OnConfirmListener() {
                                        ...
                                        public void onConfirm() {
                                            String BB_STATE = "3";
                                            String SYSTEM_SERVICE_TYPE= "US_CORP_BILL_BASE";
                                            ...
                                            viewModel.updataSellListStatues(upParamer).observe(ArrivalInboundDetailActivity.this, new Observer<JsonObject>() {
                                                ...
                                                public void onChanged(JsonObject jsonObject) {
                                                    if (jsonObject == null){
                                                        ...
                                                        if (code.equals("0")){
                                                            ArrivalInboundOrderInfo orderInfo = orderInfos.get(0);
        ...
        if (data != null) {
            ...
            if (bundle != null) {
                if (bundle.getInt(XQRCode.RESULT_TYPE) == XQRCode.RESULT_SUCCESS) {
                    ...
                    int count = 0;
                    List<ArrivalInboundBarcode> barcodes = LitePal.where("barcode = ?", result).find(ArrivalInboundBarcode.class);
                    if (barcodes.size() > 0){
                        playSound(2);
                    ...
                    for (int i = 0; i < detailList2.size(); i++){
                        ...
                        if (pcigCode.equals(code)){
                            ...
                            ArrivalInboundBarcode salesBarcode = new ArrivalInboundBarcode(uuid, pcigCode, pcigName, result, scantime, scancode);
                            salesBarcode.save();
