package com.nti.module_arrivalinbound.repository;
 ...
 * @describe
 */
public class ArrivalInboundRepo {
    public MutableLiveData<List<ArrivalInboundOrderInfo>> PDA_H(Paramer paramer){
        final MutableLiveData<List<ArrivalInboundOrderInfo>> data = new MutableLiveData<>();
        final List<ArrivalInboundOrderInfo> orderInfos = new ArrayList<>();
        HttpUtils.getInstance().with(IService.class).PDA_H(paramer)
                ...
                .subscribe(new Observer<JsonObject>() {
                    ...
                    public void onNext(@NotNull JsonObject jsonObject) {
                        ...
                        List<ArrivalInboundDetail> salesFactoryDetails = new ArrayList<>();
                        try {
                            for (int i = 0; i < jsonArray1.size(); i++){
                                ...
                                List<ArrivalInboundBarcode> barcodes = LitePal.where("UUID = ? and pcigcode = ? and isSubmit = 0", uuid, pickCode).find(ArrivalInboundBarcode.class);
                                ...
                                ArrivalInboundDetail salesFactoryDetail = new ArrivalInboundDetail(pickName, billPNum, billBNum, uuid, pickCode, scanBNum, String.valueOf(mscanQty));
                                salesFactoryDetails.add(salesFactoryDetail);
                            }
                            LitePal.deleteAll(ArrivalInboundDetail.class);
                        ...
                        try {
                            ...
                            /*if (object.has("BB_TOTAL_SCAN_NUM")){
                                ...
                                List<ArrivalInboundDetail> details = LitePal.where("BD_BB_UUID = ?", BB_UUID).find(ArrivalInboundDetail.class);
                                ...
                                ArrivalInboundOrderInfo orderInfo = new ArrivalInboundOrderInfo(BB_UUID, BB_CONTRACT_NO, BB_BT_CODE, BB_TICKET_NO, BB_WS_CODE,
                                        BB_RELATE_CONTRACT_NO, B_NAME, BB_INPUT_DATE, BB_TOTAL_ALL_NUM1, BB_TOTAL_SCAN_NUM, BB_TOTAL_PNUM, BB_FLOW_NAME, BB_STATE);
                                orderInfos.add(orderInfo);
                            }
                            LitePal.deleteAll(ArrivalInboundOrderInfo.class);
