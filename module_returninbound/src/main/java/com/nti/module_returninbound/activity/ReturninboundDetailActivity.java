...
public class ReturninboundDetailActivity extends BaseActivity implements View.OnClickListener {
    ...
    private void loadLatelyData() {
        ReturninboundOrderInfo info = LitePal.where("BB_UUID = ?", uuid).findFirst(ReturninboundOrderInfo.class);
        if (info!=null){
            A_NO = info.getA_NO();
        }
    }
    ...
    protected void onStart() {
        ...
        ReturninboundOrderInfo orderInfo = LitePal.where("BB_UUID = ?", uuid).findFirst(ReturninboundOrderInfo.class);
    ...
    public void onClick(View view) {
        ...
        if (view.getId() == R.id.confirm_brn){
            ...
            int barcode_count = LitePal.where("UUID = ? and isSubmit = 0", uuid).count(ReturninboundBarcode.class);
            if (barcode_count == 0){
    ...
    private void handleScannerResult(String result) {
        ...
        int count = 0;
        int size = LitePal.where("barcode = ?", result).count(ReturninboundBarcode.class);
        ...
        for (int i = 0; i < detailList2.size(); i++) {
            ...
            if (pcigcodesub.equals(code)) {
                ...
                ReturninboundBarcode ReturninboundBarcode = new ReturninboundBarcode(uuid, pcigCode, pcigName, result, scantime, scancode);
                ReturninboundBarcode.save();
                ...
                sellBarcodeReciveViewModel.sellBarcodeRecive(paramer).observe(this, new Observer<DataResult<JsonObject>>() {
                    ...
                    public void onChanged(DataResult<JsonObject> dataResult) {
                        ...
                        if (errcode == 0) {
                            ...
                            ReturninboundBarcode barcode = new ReturninboundBarcode(uuid, pcigCode, picgname, result, scantime, scancode);
                            barcode.setSubmit(true);
                            barcode.save();
                            Toast.makeText(ReturninboundDetailActivity.this, "sucess", Toast.LENGTH_SHORT).show();
