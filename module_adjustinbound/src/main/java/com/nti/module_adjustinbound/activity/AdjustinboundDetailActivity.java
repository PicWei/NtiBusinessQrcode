...
import com.nti.lib_common.bean.AdjustinboundBarcode;
...
import com.nti.module_adjustinbound.adapter.AdjustinboundDetailAdapter;
import com.nti.module_adjustinbound.databinding.ActivityAdjustinboundBinding;
import com.nti.module_adjustinbound.databinding.ActivityAdjustinboundDetailBinding;
...
//商业调剂入库
@Route(path = ARouterPath.ADJUSTINBOUNDDETAIL_PATH)
public class AdjustinboundDetailActivity extends BaseActivity implements View.OnClickListener {

    private ActivityAdjustinboundDetailBinding binding;
    private AdjustinboundDetailAdapter adapter;

    private List<AdjustinboundDetail> detailList = new ArrayList<>();

    private List<AdjustinboundDetail> detailList2 = new ArrayList<>();
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        binding = DataBindingUtil.setContentView(this, R.layout.activity_adjustinbound_detail);
    ...
    private void loadLatelyData() {
        AdjustinboundOrderInfo info = LitePal.where("BB_UUID = ?", uuid).findFirst(AdjustinboundOrderInfo.class);
        if (info!=null){
            A_NO = info.getA_NO();
        }
    }
    ...
    protected void onStart() {
        ...
        AdjustinboundOrderInfo orderInfo = LitePal.where("BB_UUID = ?", uuid).findFirst(AdjustinboundOrderInfo.class);
        ...
        detailList2 = LitePal.where("BD_BB_UUID = ?", uuid).find(AdjustinboundDetail.class);

        detailList.addAll(detailList2);
        adapter = new AdjustinboundDetailAdapter(detailList, this);
        binding.recyclerView.setAdapter(adapter);
        adapter.setOnItemClickListener(new AdjustinboundDetailAdapter.OnItemClickListener() {
            @Override
            public void onItemClick(AdjustinboundDetail detail) {
                String pcigcode = detail.getBD_PCIG_CODE();
                List<AdjustinboundBarcode> barcodes = LitePal.where("pcigcode = ? and isSubmit = 0", pcigcode).find(AdjustinboundBarcode.class);
                Collections.sort(barcodes, new Comparator<AdjustinboundBarcode>() {
                    @Override
                    public int compare(AdjustinboundBarcode salesBarcode, AdjustinboundBarcode t1) {
                        return t1.getScantime().compareTo(salesBarcode.getScantime());
                    }
                });
                BasePopupView popupView = new BarcodeListPopup(AdjustinboundDetailActivity.this);
                new XPopup.Builder(AdjustinboundDetailActivity.this)
    ...
    public void onClick(View view) {
        ...
        if (view.getId() == R.id.confirm_brn){
            ...
            int barcode_count = LitePal.where("UUID = ? and isSubmit = 0", uuid).count(AdjustinboundBarcode.class);
            if (barcode_count == 0){
                            ...
                            .asConfirm(null, "当前单据还未扫描完全，确认完成？",
                                    ...
                                    new OnConfirmListener() {
                                        ...
                                        public void onConfirm() {
                                            ...
                                            ViewModel.updataSellListStatues(upParamer).observe(AdjustinboundDetailActivity.this, new Observer<JsonObject>() {
                                                @Override
                                                public void onChanged(JsonObject jsonObject) {
                                                    if (jsonObject == null){
                                                        Toast.makeText(AdjustinboundDetailActivity.this, "网络异常", Toast.LENGTH_SHORT).show();
                                                    }else {
                                                        String code = jsonObject.get("code").toString().replace("\"", "");
                                                        if (code.equals("0")){
                                                            orderInfo.setBB_STATE("3");
                                                            orderInfo.saveOrUpdate("BB_UUID = ?", uuid);
                                                            Toast.makeText(AdjustinboundDetailActivity.this, "确认成功", Toast.LENGTH_SHORT).show();
                                                        }else {
                                                            Toast.makeText(AdjustinboundDetailActivity.this, "更改状态失败", Toast.LENGTH_SHORT).show();
                                                        }
                                                    }
                                                }
                                            });
    ...
    private void handleScannerResult(String result) {
        ...
        int count = 0;
        int size = LitePal.where("barcode = ?", result).count(AdjustinboundBarcode.class);
        ...
        for (int i = 0; i < detailList2.size(); i++) {
            ...
            AdjustinboundDetail detail = detailList2.get(i);
            String pcigCode = detail.getBD_PCIG_CODE();
            ...
            if (pcigcodesub.equals(code)) {
                ...
                AdjustinboundBarcode AdjustinboundBarcode = new AdjustinboundBarcode(uuid, pcigCode, pcigName, result, scantime, scancode);
                AdjustinboundBarcode.save();
                String new_scanQty = String.valueOf(mscanQty);
                ContentValues cv = new ContentValues();
                cv.put("BD_SCAN_NUM", new_scanQty);
                LitePal.updateAll(AdjustinboundDetail.class, cv, "BD_PCIG_CODE = ?", pcigCode);
                ContentValues cv2 = new ContentValues();
                List<AdjustinboundOrderInfo> orderInfos = LitePal.where("BB_UUID = ?", uuid).find(AdjustinboundOrderInfo.class);
                ...
                LitePal.updateAll(AdjustinboundOrderInfo.class, cv2, "BB_UUID = ?", uuid);
                ...
                sellBarcodeReciveViewModel.sellBarcodeRecive(paramer).observe(this, new Observer<DataResult<JsonObject>>() {
                    ...
                    public void onChanged(DataResult<JsonObject> dataResult) {
                        ...
                        if (errcode == 0) {
                            ...
                            AdjustinboundBarcode barcode = new AdjustinboundBarcode(uuid, pcigCode, picgname, result, scantime, scancode);
                            barcode.setSubmit(true);
                            barcode.save();
                            Toast.makeText(AdjustinboundDetailActivity.this, "sucess", Toast.LENGTH_SHORT).show();
        ...
        ViewModel.errorSignReceive(esrparamer).observe(this, new Observer<JsonObject>() {
            ...
            public void onChanged(JsonObject jsonObject) {
                if (jsonObject == null){
                    ...
                    Toast.makeText(AdjustinboundDetailActivity.this, "异常条码上传成功", Toast.LENGTH_SHORT).show();
                }
            }
        });
    }
