...
public class AdjustinboundActivity extends BaseActivity implements View.OnClickListener {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        viewModel.PDA_H(paramer).observe(this, new Observer<List<AdjustinboundOrderInfo>>() {
            @Override
            public void onChanged(List<AdjustinboundOrderInfo> AdjustinboundOrderInfos) {
                loadingPopup.dismiss();
                if (AdjustinboundOrderInfos == null){
                    Toast.makeText(AdjustinboundActivity.this, "数据为空", Toast.LENGTH_SHORT).show();
                }else {
                    binding.incompleteCl.performClick();
                    List<AdjustinboundOrderInfo> orderInfos = LitePal.where("BB_STATE = ?", "4").find(AdjustinboundOrderInfo.class);
                    List<AdjustinboundOrderInfo> orderInfos2 = LitePal.where("BB_STATE = ?", "1").find(AdjustinboundOrderInfo.class);
                    List<AdjustinboundOrderInfo> orderInfos3 = LitePal.where("BB_STATE = ?", "3").find(AdjustinboundOrderInfo.class);
    ...
    public void onClick(View view) {
        if (view.getId() == R.id.incomplete_cl){
            ...
            viewModel.PDA_H(paramer).observe(this, new Observer<List<AdjustinboundOrderInfo>>() {
                @Override
                public void onChanged(List<AdjustinboundOrderInfo> salesFactoryOrderInfos) {
                    loadingPopup.dismiss();
    ...
    public void onMessageEvent(MessageEvent event){
        switch (event.what){
            case BusinessType.UPDATA_ONGOING:
                List<AdjustinboundOrderInfo> orderInfos2 = LitePal.where("BB_STATE = ? or BB_STATE = ? or BB_STATE = ?", "1", "3", "4").find(AdjustinboundOrderInfo.class);
                int ongoingNum = orderInfos2.size();
                binding.ongoingNum.setText(ongoingNum+"");
                break;
        }
