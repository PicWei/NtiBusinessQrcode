package com.nti.module_adjustinbound.repository;
...
import com.nti.module_adjustinbound.service.IAdjustboundinService;
 ...
 * @describe
 */
public class AdjustinboundRepo {
    public MutableLiveData<DataResult<List<AdjustinboundOrderInfo>>> PDA_H(Paramer paramer){
        final MutableLiveData<DataResult<List<AdjustinboundOrderInfo>>> data = new MutableLiveData<>();
        final List<AdjustinboundOrderInfo> orderInfos = new ArrayList<>();
        HttpUtils.getInstance().with(IAdjustboundinService.class).PDA_H(paramer)
                ...
                .subscribe(new Observer<JsonObject>() {
                    ...
                    public void onNext(@NotNull JsonObject jsonObject) {
                        ...
                        List<AdjustinboundDetail> salesFactoryDetails = new ArrayList<>();
                        try {
                            for (int i = 0; i < jsonArray1.size(); i++){
                                ...
                                try {
                                    List<AdjustinboundBarcode> barcodes = LitePal.where("UUID = ? and pcigcode = ? and isSubmit = 0", uuid, pickCode).find(AdjustinboundBarcode.class);
                                    ...
                                    AdjustinboundDetail salesFactoryDetail = new AdjustinboundDetail(pickName, billPNum, billBNum, uuid, pickCode, scanBNum, String.valueOf(mscanQty));
                        ...
                        try {
                            LitePal.deleteAll(AdjustinboundDetail.class);
                        ...
                        try {
                            for (int i = 0; i < jsonArray.size(); i++){
                                ...
                                try {
                                    List<AdjustinboundDetail> details = LitePal.where("BD_BB_UUID = ?", BB_UUID).find(AdjustinboundDetail.class);
                                    ...
                                    AdjustinboundOrderInfo orderInfo = new AdjustinboundOrderInfo(BB_UUID, BB_CONTRACT_NO, BB_BT_CODE, BB_TICKET_NO, BB_WS_CODE,
                        ...
                        try {
                            LitePal.deleteAll(AdjustinboundOrderInfo.class);
                            LitePal.saveAll(orderInfos);
                            //                   data.setValue(orderInfos);
                            DataResult<List<AdjustinboundOrderInfo>> dataResult = new DataResult<>(0, orderInfos);
                    ...
                    public void onError(@NotNull Throwable e) {
                        DataResult<List<AdjustinboundOrderInfo>> dataResult = new DataResult<>(-1, null);
                        data.setValue(dataResult);
                    }
    ...
    public MutableLiveData<JsonObject> updataSellListStatues(UpParamer paramer){
        ...
        HttpUtils.getInstance().with(IAdjustboundinService.class).updataSellListStatues(paramer)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Observer<JsonObject>() {
                    @Override
    ...
    public MutableLiveData<JsonObject> errorSignReceive(ErrorSignReceiveParamer paramer){
        final MutableLiveData<JsonObject> data = new MutableLiveData<>();
        HttpUtils.getInstance().with(IAdjustboundinService.class).errorSignReceive(paramer)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Observer<JsonObject>() {
                    @Override
