package com.nti.module_adjustoutbound.repository;
...
import com.nti.module_adjustoutbound.service.IAdjustboundoutService;
 ...
 * @describe
 */
public class AdjustoutboundRepo {
    public MutableLiveData<DataResult<List<AdjustoutboundOrderInfo>>> PDA_H(Paramer paramer){
        final MutableLiveData<DataResult<List<AdjustoutboundOrderInfo>>> data = new MutableLiveData<>();
        final List<AdjustoutboundOrderInfo> orderInfos = new ArrayList<>();
        HttpUtils.getInstance().with(IAdjustboundoutService.class).PDA_H(paramer)
                ...
                .subscribe(new Observer<JsonObject>() {
                    ...
                    public void onNext(@NotNull JsonObject jsonObject) {
                        ...
                        try {
                            for (int i = 0; i < jsonArray.size(); i++){
                                ...
                                try {
                                    ...
                                    AdjustoutboundOrderInfo orderInfo = new AdjustoutboundOrderInfo(BB_UUID, BB_CONTRACT_NO, BB_BT_CODE, BB_TICKET_NO, BB_WS_CODE,
                        ...
                        try {
                            LitePal.deleteAll(AdjustoutboundOrderInfo.class);
                            LitePal.saveAll(orderInfos);
                            //                   data.setValue(orderInfos);
                            DataResult<List<AdjustoutboundOrderInfo>> dataResult = new DataResult<>(0, orderInfos);
                    ...
                    public void onError(@NotNull Throwable e) {
                        DataResult<List<AdjustoutboundOrderInfo>> dataResult = new DataResult<>(-1, null);
                        data.setValue(dataResult);
                    }
    ...
    public MutableLiveData<JsonObject> updataSellListStatues(UpParamer paramer){
        ...
        HttpUtils.getInstance().with(IAdjustboundoutService.class).updataSellListStatues(paramer)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Observer<JsonObject>() {
                    @Override
    ...
    public MutableLiveData<JsonObject> errorSignReceive(ErrorSignReceiveParamer paramer){
        final MutableLiveData<JsonObject> data = new MutableLiveData<>();
        HttpUtils.getInstance().with(IAdjustboundoutService.class).errorSignReceive(paramer)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Observer<JsonObject>() {
                    @Override
