...
public class AdjustoutboundDetailActivity extends BaseActivity implements View.OnClickListener{
    ...
    public void onClick(View view) {
        ...
        if (view.getId() == R.id.confirm_brn){
            ...
            List<AdjustoutboundBarcode> barcodes = LitePal.where("UUID = ? and isSubmit = 0", uuid).find(AdjustoutboundBarcode.class);
            if (barcodes.size() == 0){
                                            ...
                                            viewModel.updataSellListStatues(upParamer).observe(AdjustoutboundDetailActivity.this, new Observer<JsonObject>() {
                                                ...
                                                public void onChanged(JsonObject jsonObject) {
                                                    if (jsonObject == null){
                                                        ...
                                                        if (code.equals("0")){
                                                            AdjustoutboundOrderInfo orderInfo = orderInfos.get(0);
        ...
        if (data != null) {
            ...
            if (bundle != null) {
                if (bundle.getInt(XQRCode.RESULT_TYPE) == XQRCode.RESULT_SUCCESS) {
                    ...
                    int count = 0;
                    List<AdjustoutboundBarcode> barcodes = LitePal.where("barcode = ?", result).find(AdjustoutboundBarcode.class);
                    if (barcodes.size() > 0){
                        playSound(2);
                    ...
                    for (int i = 0; i < detailList2.size(); i++){
                        ...
                        if (pcigCode.equals(code)){
                            ...
                            AdjustoutboundBarcode salesBarcode = new AdjustoutboundBarcode(uuid, pcigCode, pcigName, result, scantime, scancode);
                            salesBarcode.save();
