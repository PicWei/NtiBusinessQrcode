package com.nti.module_adjustoutbound.activity;
...
import com.nti.module_adjustoutbound.adapter.AdjustoutboundDetailAdapter;
import com.nti.module_adjustoutbound.databinding.ActivityAdjustoutboundDetailBinding;
...
@Route(path = ARouterPath.ADJUSTOUTBOUNDDETAIL_PATH)
public class AdjustoutboundDetailActivity extends BaseActivity implements View.OnClickListener{

    private ActivityAdjustoutboundDetailBinding binding;
    private AdjustoutboundDetailAdapter adapter;

    private List<AdjustoutboundDetail> detailList = new ArrayList<>();

    private List<AdjustoutboundDetail> detailList2 = new ArrayList<>();
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        binding = DataBindingUtil.setContentView(this, R.layout.activity_adjustoutbound_detail);
    ...
    private void loadLatelyData() {
        AdjustoutboundOrderInfo info = LitePal.where("BB_UUID = ?", uuid).findFirst(AdjustoutboundOrderInfo.class);
        if (info!=null){
            A_NO = info.getA_NO();
        }
    }
    ...
    protected void onStart() {
        ...
        AdjustoutboundOrderInfo orderInfo = LitePal.where("BB_UUID = ?", uuid).findFirst(AdjustoutboundOrderInfo.class);
        ...
        detailList2 = LitePal.where("BD_BB_UUID = ?", uuid).find(AdjustoutboundDetail.class);

        detailList.addAll(detailList2);
        adapter = new AdjustoutboundDetailAdapter(detailList, this);
        binding.recyclerView.setAdapter(adapter);
        adapter.setOnItemClickListener(new AdjustoutboundDetailAdapter.OnItemClickListener() {
            @Override
            public void onItemClick(AdjustoutboundDetail detail) {
                String pcigcode = detail.getBD_PCIG_CODE();
                List<AdjustoutboundBarcode> barcodes = LitePal.where("pcigcode = ? and isSubmit = 0", pcigcode).find(AdjustoutboundBarcode.class);
                Collections.sort(barcodes, new Comparator<AdjustoutboundBarcode>() {
                    @Override
                    public int compare(AdjustoutboundBarcode salesBarcode, AdjustoutboundBarcode t1) {
                        return t1.getScantime().compareTo(salesBarcode.getScantime());
                    }
                });
                BasePopupView popupView = new BarcodeListPopup(AdjustoutboundDetailActivity.this);
                new XPopup.Builder(AdjustoutboundDetailActivity.this)
    ...
    public void onClick(View view) {
        ...
        if (view.getId() == R.id.confirm_brn){
            ...
            int barcode_count = LitePal.where("UUID = ? and isSubmit = 0", uuid).count(AdjustoutboundBarcode.class);
            if (barcode_count == 0){
                            ...
                            .asConfirm(null, "当前单据还未扫描完全，确认完成？",
                                    ...
                                    new OnConfirmListener() {
                                        ...
                                        public void onConfirm() {
                                            ...
                                            ViewModel.updataSellListStatues(upParamer).observe(AdjustoutboundDetailActivity.this, new Observer<JsonObject>() {
                                                @Override
                                                public void onChanged(JsonObject jsonObject) {
                                                    if (jsonObject == null){
                                                        Toast.makeText(AdjustoutboundDetailActivity.this, "网络异常", Toast.LENGTH_SHORT).show();
                                                    }else {
                                                        String code = jsonObject.get("code").toString().replace("\"", "");
                                                        if (code.equals("0")){
                                                            orderInfo.setBB_STATE("3");
                                                            orderInfo.saveOrUpdate("BB_UUID = ?", uuid);
                                                            Toast.makeText(AdjustoutboundDetailActivity.this, "确认成功", Toast.LENGTH_SHORT).show();
                                                        }else {
                                                            Toast.makeText(AdjustoutboundDetailActivity.this, "更改状态失败", Toast.LENGTH_SHORT).show();
                                                        }
                                                    }
                                                }
                                            });
    ...
    private void handleScannerResult(String result) {
        ...
        int count = 0;
        int size = LitePal.where("barcode = ?", result).count(AdjustoutboundBarcode.class);
        ...
        for (int i = 0; i < detailList2.size(); i++) {
            ...
            AdjustoutboundDetail detail = detailList2.get(i);
            String pcigCode = detail.getBD_PCIG_CODE();
            ...
            if (pcigcodesub.equals(code)) {
                ...
                AdjustoutboundBarcode AdjustoutboundBarcode = new AdjustoutboundBarcode(uuid, pcigCode, pcigName, result, scantime, scancode);
                AdjustoutboundBarcode.save();
                String new_scanQty = String.valueOf(mscanQty);
                ContentValues cv = new ContentValues();
                cv.put("BD_SCAN_NUM", new_scanQty);
                LitePal.updateAll(AdjustoutboundDetail.class, cv, "BD_PCIG_CODE = ?", pcigCode);
                ContentValues cv2 = new ContentValues();
                List<AdjustoutboundOrderInfo> orderInfos = LitePal.where("BB_UUID = ?", uuid).find(AdjustoutboundOrderInfo.class);
                ...
                LitePal.updateAll(AdjustoutboundOrderInfo.class, cv2, "BB_UUID = ?", uuid);
                ...
                sellBarcodeReciveViewModel.sellBarcodeRecive(paramer).observe(this, new Observer<DataResult<JsonObject>>() {
                    ...
                    public void onChanged(DataResult<JsonObject> dataResult) {
                        ...
                        if (errcode == 0) {
                            ...
                            AdjustoutboundBarcode barcode = new AdjustoutboundBarcode(uuid, pcigCode, picgname, result, scantime, scancode);
                            barcode.setSubmit(true);
                            barcode.save();
                            Toast.makeText(AdjustoutboundDetailActivity.this, "sucess", Toast.LENGTH_SHORT).show();
        ...
        ViewModel.errorSignReceive(esrparamer).observe(this, new Observer<JsonObject>() {
            ...
            public void onChanged(JsonObject jsonObject) {
                if (jsonObject == null){
                    ...
                    Toast.makeText(AdjustoutboundDetailActivity.this, "异常条码上传成功", Toast.LENGTH_SHORT).show();
                }
            }
        });
    }
