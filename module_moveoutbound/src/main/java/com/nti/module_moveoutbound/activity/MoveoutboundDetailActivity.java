...
public class MoveoutboundDetailActivity extends BaseActivity implements View.OnClickListener {
    ...
    public void onClick(View view) {
        ...
        if (view.getId() == R.id.confirm_brn){
            ...
            List<MoveoutboundBarcode> barcodes = LitePal.where("UUID = ? and isSubmit = 0", uuid).find(MoveoutboundBarcode.class);
            if (barcodes.size() == 0){
                            ...
                            .asConfirm(null, "当前单据还未扫描完全，确认完成？",
                                    ...
                                    new OnConfirmListener() {
                                        ...
                                        public void onConfirm() {
                                            ...
                                            viewModel.updataSellListStatues(upParamer).observe(MoveoutboundDetailActivity.this, new Observer<JsonObject>() {
                                                @Override
                                                public void onChanged(JsonObject jsonObject) {
                                                    if (jsonObject == null){
                                                        ...
                                                        if (code.equals("0")){
                                                            MoveoutboundOrderInfo orderInfo = orderInfos.get(0);
        ...
        if (data != null) {
            ...
            if (bundle != null) {
                if (bundle.getInt(XQRCode.RESULT_TYPE) == XQRCode.RESULT_SUCCESS) {
                    ...
                    int count = 0;
                    List<MoveoutboundBarcode> barcodes = LitePal.where("barcode = ?", result).find(MoveoutboundBarcode.class);
                    if (barcodes.size() > 0){
                        playSound(2);
                    ...
                    for (int i = 0; i < detailList2.size(); i++){
                        ...
                        if (pcigCode.equals(code)){
                            ...
                            MoveoutboundBarcode salesBarcode = new MoveoutboundBarcode(uuid, pcigCode, pcigName, result, scantime, scancode);
                            salesBarcode.save();
