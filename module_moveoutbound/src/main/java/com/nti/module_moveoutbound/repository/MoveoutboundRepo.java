package com.nti.module_moveoutbound.repository;
...
import com.nti.lib_common.bean.MoveoutboundBarcode;
import com.nti.lib_common.bean.MoveoutboundBarcode;
import com.nti.lib_common.bean.MoveoutboundDetail;
import com.nti.lib_common.bean.MoveoutboundDetail;
 ...
 * @describe
 */
public class MoveoutboundRepo {
    public MutableLiveData<List<MoveoutboundOrderInfo>> PDA_H(Paramer paramer){
        final MutableLiveData<List<MoveoutboundOrderInfo>> data = new MutableLiveData<>();
        ...
        HttpUtils.getInstance().with(IService.class).PDA_H(paramer)
                ...
                .subscribe(new Observer<JsonObject>() {
                    ...
                    public void onNext(@NotNull JsonObject jsonObject) {
                        ...
                        List<MoveoutboundDetail> salesFactoryDetails = new ArrayList<>();
                        try {
                            for (int i = 0; i < jsonArray1.size(); i++){
                                ...
                                List<MoveoutboundBarcode> barcodes = LitePal.where("UUID = ? and pcigcode = ? and isSubmit = 0", uuid, pickCode).find(MoveoutboundBarcode.class);
                                ...
                                MoveoutboundDetail salesFactoryDetail = new MoveoutboundDetail(pickName, billPNum, billBNum, uuid, pickCode, scanBNum, String.valueOf(mscanQty));
                        ...
                        try {
                            for (int i = 0; i < jsonArray.size(); i++){
                                ...
                                MoveoutboundOrderInfo orderInfo = new MoveoutboundOrderInfo(BB_UUID, BB_CONTRACT_NO, BB_BT_CODE, BB_TICKET_NO, BB_WS_CODE,
                                        BB_RELATE_CONTRACT_NO, B_NAME, BB_INPUT_DATE, BB_TOTAL_ALL_NUM1, BB_TOTAL_SCAN_NUM, BB_TOTAL_PNUM, BB_FLOW_NAME, BB_STATE);
                                orderInfos.add(orderInfo);
                            }
                            LitePal.deleteAll(MoveoutboundOrderInfo.class);
