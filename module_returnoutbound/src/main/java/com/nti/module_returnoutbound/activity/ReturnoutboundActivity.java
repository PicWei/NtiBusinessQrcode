package com.nti.module_returnoutbound.activity;
...
import com.nti.module_returnoutbound.databinding.ActivityReturnoutboundBinding;
import com.nti.module_returnoutbound.fragment.CompletedFragment;
import com.nti.module_returnoutbound.fragment.IncompleteFragment;
import com.nti.module_returnoutbound.fragment.OngoingFragment;
...
//商业退货出库 US_BACK_BILL_BASE
@Route(path = ARouterPath.RETURNOUTBOUND_PATH)
public class ReturnoutboundActivity extends BaseActivity implements View.OnClickListener{

    private ActivityReturnoutboundBinding binding;
    private ReturnoutboundVM viewModel;
    private LoadingPopupView loadingPopup;
    private int current = 1;

    public static final String SYSTEM_SERVICE_TYPE = "US_BACK_BILL_BASE";
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        binding = DataBindingUtil.setContentView(this, R.layout.activity_returnoutbound);
    ...
    private void loadData() {
        ...
        viewModel.PDA_H(paramer).observe(this, new Observer<DataResult<List<ReturnoutboundOrderInfo>>>() {
            @Override
            public void onChanged(DataResult<List<ReturnoutboundOrderInfo>> dataResult) {
                loadingPopup.dismiss();
                int errcode = dataResult.getErrcode();
                if (errcode == -1){
                    Toast.makeText(ReturnoutboundActivity.this, "网络异常", Toast.LENGTH_SHORT).show();
                }else {
                    List<ReturnoutboundOrderInfo> ReturnoutboundOrderInfos = dataResult.getT();
                    if (ReturnoutboundOrderInfos == null || ReturnoutboundOrderInfos.isEmpty()){
                        Toast.makeText(ReturnoutboundActivity.this, "数据为空", Toast.LENGTH_SHORT).show();
                    }else {
                        binding.incompleteCl.performClick();
                        loadViewCount();
                    }
    ...
    private void loadViewCount() {
        unFinishCount = LitePal.where("BB_STATE = ?", "4").count(ReturnoutboundOrderInfo.class);
        doingCount = LitePal.where("BB_STATE = ?", "1").count(ReturnoutboundOrderInfo.class);
        finishCount = LitePal.where("BB_STATE = ? and PDA_SCANNER_IS_END = ?", "3", "0").count(ReturnoutboundOrderInfo.class);
