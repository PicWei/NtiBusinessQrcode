...
public class ReturnoutboundActivity extends BaseActivity implements View.OnClickListener{
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        viewModel.PDA_H(paramer).observe(this, new Observer<List<ReturnoutboundOrderInfo>>() {
            @Override
            public void onChanged(List<ReturnoutboundOrderInfo> ReturnoutboundOrderInfos) {
                loadingPopup.dismiss();
                if (ReturnoutboundOrderInfos == null){
                    Toast.makeText(ReturnoutboundActivity.this, "数据为空", Toast.LENGTH_SHORT).show();
                }else {
                    binding.incompleteCl.performClick();
                    List<ReturnoutboundOrderInfo> orderInfos = LitePal.where("BB_STATE = ?", "4").find(ReturnoutboundOrderInfo.class);
                    List<ReturnoutboundOrderInfo> orderInfos2 = LitePal.where("BB_STATE = ?", "1").find(ReturnoutboundOrderInfo.class);
                    List<ReturnoutboundOrderInfo> orderInfos3 = LitePal.where("BB_STATE = ?", "3").find(ReturnoutboundOrderInfo.class);
    ...
    public void onClick(View view) {
        if (view.getId() == R.id.incomplete_cl){
            ...
            viewModel.PDA_H(paramer).observe(this, new Observer<List<ReturnoutboundOrderInfo>>() {
                @Override
                public void onChanged(List<ReturnoutboundOrderInfo> salesFactoryOrderInfos) {
                    loadingPopup.dismiss();
    ...
    public void onMessageEvent(MessageEvent event){
        switch (event.what){
            case BusinessType.UPDATA_ONGOING:
                List<ReturnoutboundOrderInfo> orderInfos2 = LitePal.where("BB_STATE = ? or BB_STATE = ? or BB_STATE = ?", "1", "3", "4").find(ReturnoutboundOrderInfo.class);
                int ongoingNum = orderInfos2.size();
                binding.ongoingNum.setText(ongoingNum+"");
                break;
        }
