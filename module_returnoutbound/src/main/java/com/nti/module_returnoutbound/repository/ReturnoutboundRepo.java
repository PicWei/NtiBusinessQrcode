package com.nti.module_returnoutbound.repository;
 ...
 * @describe
 */
public class ReturnoutboundRepo {
    public MutableLiveData<List<ReturnoutboundOrderInfo>> PDA_H(Paramer paramer){
        final MutableLiveData<List<ReturnoutboundOrderInfo>> data = new MutableLiveData<>();
        final List<ReturnoutboundOrderInfo> orderInfos = new ArrayList<>();
        HttpUtils.getInstance().with(IService.class).PDA_H(paramer)
                ...
                .subscribe(new Observer<JsonObject>() {
                    ...
                    public void onNext(@NotNull JsonObject jsonObject) {
                        ...
                        List<ReturnoutboundDetail> salesFactoryDetails = new ArrayList<>();
                        try {
                            for (int i = 0; i < jsonArray1.size(); i++){
                                ...
                                List<ReturnoutboundBarcode> barcodes = LitePal.where("UUID = ? and pcigcode = ? and isSubmit = 0", uuid, pickCode).find(ReturnoutboundBarcode.class);
                                ...
                                ReturnoutboundDetail salesFactoryDetail = new ReturnoutboundDetail(pickName, billPNum, billBNum, uuid, pickCode, scanBNum, String.valueOf(mscanQty));
                        ...
                        try {
                            ...
                            /*if (object.has("BB_TOTAL_SCAN_NUM")){
                                ...
                                List<ReturnoutboundDetail> details = LitePal.where("BD_BB_UUID = ?", BB_UUID).find(ReturnoutboundDetail.class);
                                ...
                                ReturnoutboundOrderInfo orderInfo = new ReturnoutboundOrderInfo(BB_UUID, BB_CONTRACT_NO, BB_BT_CODE, BB_TICKET_NO, BB_WS_CODE,
                                        BB_RELATE_CONTRACT_NO, B_NAME, BB_INPUT_DATE, BB_TOTAL_ALL_NUM1, BB_TOTAL_SCAN_NUM, BB_TOTAL_PNUM, BB_FLOW_NAME, BB_STATE);
                                orderInfos.add(orderInfo);
                            }
                            LitePal.deleteAll(ReturnoutboundOrderInfo.class);
