package com.nti.module_sortinginbound.activity;
...
import com.nti.lib_common.bean.SortinginboundBarcode;
import com.nti.lib_common.bean.SortinginboundDetail;
...
import com.nti.module_sortinginbound.adapter.SortinginboundDetailAdapter;
import com.nti.module_sortinginbound.databinding.ActivitySortinginboundDetailBinding;
import com.nti.module_sortinginbound.viewmodel.SortinginboundVM;
...
@Route(path = ARouterPath.SORTINGINBOUNDDETAIL_PATH)
public class SortinginboundDetailActivity extends BaseActivity implements View.OnClickListener {

    private ActivitySortinginboundDetailBinding binding;
    private SortinginboundDetailAdapter adapter;

    private List<SortinginboundDetail> detailList = new ArrayList<>();

    private List<SortinginboundDetail> detailList2 = new ArrayList<>();
    ...
    public static final String SYSTEM_SERVICE_TYPE = "US_SBACK_BILL_BASE";


    private SortinginboundVM ViewModel;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        binding = DataBindingUtil.setContentView(this, R.layout.activity_sortinginbound_detail);
        ...
        ViewModel = new ViewModelProvider(this).get(SortinginboundVM.class);
    ...
    private void loadLatelyData() {
        SortinginboundOrderInfo info = LitePal.where("BB_UUID = ?", uuid).findFirst(SortinginboundOrderInfo.class);
        if (info!=null){
            A_NO = info.getA_NO();
        }
    }
    ...
    protected void onStart() {
        ...
        SortinginboundOrderInfo orderInfo = LitePal.where("BB_UUID = ?", uuid).findFirst(SortinginboundOrderInfo.class);
        ...
        detailList2 = LitePal.where("BD_BB_UUID = ?", uuid).find(SortinginboundDetail.class);

        detailList.addAll(detailList2);
        adapter = new SortinginboundDetailAdapter(detailList, this);
        binding.recyclerView.setAdapter(adapter);
        adapter.setOnItemClickListener(new SortinginboundDetailAdapter.OnItemClickListener() {
            @Override
            public void onItemClick(SortinginboundDetail detail) {
                String pcigcode = detail.getBD_PCIG_CODE();
                List<SortinginboundBarcode> barcodes = LitePal.where("pcigcode = ? and isSubmit = 0", pcigcode).find(SortinginboundBarcode.class);
                Collections.sort(barcodes, new Comparator<SortinginboundBarcode>() {
                    @Override
                    public int compare(SortinginboundBarcode salesBarcode, SortinginboundBarcode t1) {
                        return t1.getScantime().compareTo(salesBarcode.getScantime());
                    }
                });
                BasePopupView popupView = new BarcodeListPopup(SortinginboundDetailActivity.this);
                new XPopup.Builder(SortinginboundDetailActivity.this)
    ...
    public void onClick(View view) {
        ...
        if (view.getId() == R.id.confirm_brn){
            ...
            int barcode_count = LitePal.where("UUID = ? and isSubmit = 0", uuid).count(SortinginboundBarcode.class);
            if (barcode_count == 0){
                            ...
                            .asConfirm(null, "当前单据还未扫描完全，确认完成？",
                                    ...
                                    new OnConfirmListener() {
                                        ...
                                        public void onConfirm() {
                                            ...
                                            ViewModel.updataSellListStatues(upParamer).observe(SortinginboundDetailActivity.this, new Observer<JsonObject>() {
                                                @Override
                                                public void onChanged(JsonObject jsonObject) {
                                                    if (jsonObject == null){
                                                        Toast.makeText(SortinginboundDetailActivity.this, "网络异常", Toast.LENGTH_SHORT).show();
                                                    }else {
                                                        String code = jsonObject.get("code").toString().replace("\"", "");
                                                        if (code.equals("0")){
                                                            orderInfo.setBB_STATE("3");
                                                            orderInfo.saveOrUpdate("BB_UUID = ?", uuid);
                                                            Toast.makeText(SortinginboundDetailActivity.this, "确认成功", Toast.LENGTH_SHORT).show();
                                                        }else {
                                                            Toast.makeText(SortinginboundDetailActivity.this, "更改状态失败", Toast.LENGTH_SHORT).show();
                                                        }
                                                    }
                                                }
                                            });
    ...
    private void handleScannerResult(String result) {
        ...
        int count = 0;
        int size = LitePal.where("barcode = ?", result).count(SortinginboundBarcode.class);
        ...
        for (int i = 0; i < detailList2.size(); i++) {
            ...
            SortinginboundDetail detail = detailList2.get(i);
            String pcigCode = detail.getBD_PCIG_CODE();
            ...
            if (pcigcodesub.equals(code)) {
                ...
                SortinginboundBarcode SortinginboundBarcode = new SortinginboundBarcode(uuid, pcigCode, pcigName, result, scantime, scancode);
                SortinginboundBarcode.save();
                String new_scanQty = String.valueOf(mscanQty);
                ContentValues cv = new ContentValues();
                cv.put("BD_SCAN_NUM", new_scanQty);
                LitePal.updateAll(SortinginboundDetail.class, cv, "BD_PCIG_CODE = ?", pcigCode);
                ContentValues cv2 = new ContentValues();
                List<SortinginboundOrderInfo> orderInfos = LitePal.where("BB_UUID = ?", uuid).find(SortinginboundOrderInfo.class);
                ...
                LitePal.updateAll(SortinginboundOrderInfo.class, cv2, "BB_UUID = ?", uuid);
                ...
                sellBarcodeReciveViewModel.sellBarcodeRecive(paramer).observe(this, new Observer<DataResult<JsonObject>>() {
                    ...
                    public void onChanged(DataResult<JsonObject> dataResult) {
                        ...
                        if (errcode == 0) {
                            ...
                            SortinginboundBarcode barcode = new SortinginboundBarcode(uuid, pcigCode, picgname, result, scantime, scancode);
                            barcode.setSubmit(true);
                            barcode.save();
                            Toast.makeText(SortinginboundDetailActivity.this, "sucess", Toast.LENGTH_SHORT).show();
        ...
        ViewModel.errorSignReceive(esrparamer).observe(this, new Observer<JsonObject>() {
            ...
            public void onChanged(JsonObject jsonObject) {
                if (jsonObject == null){
                    ...
                    Toast.makeText(SortinginboundDetailActivity.this, "异常条码上传成功", Toast.LENGTH_SHORT).show();
                }
            }
        });
    }
