...
public class SortinginboundDetailActivity extends BaseActivity implements View.OnClickListener {
    ...
    public void onClick(View view) {
        ...
        if (view.getId() == R.id.confirm_brn){
            ...
            List<SortinginboundBarcode> barcodes = LitePal.where("UUID = ? and isSubmit = 0", uuid).find(SortinginboundBarcode.class);
            if (barcodes.size() == 0){
                    ...
                    BasePopupView popupView = new XPopup.Builder(this)
                            ...
                            .asConfirm(null, "当前单据还未扫描完全，确认完成？",
                                    ...
                                    new OnConfirmListener() {
                                        ...
                                        public void onConfirm() {
                                            String BB_STATE = "3";
                                            String SYSTEM_SERVICE_TYPE= "US_SBACK_BILL_BASE";
                                            ...
                                            viewModel.updataSellListStatues(upParamer).observe(SortinginboundDetailActivity.this, new Observer<JsonObject>() {
                                                ...
                                                public void onChanged(JsonObject jsonObject) {
                                                    if (jsonObject == null){
                                                        ...
                                                        if (code.equals("0")){
                                                            SortinginboundOrderInfo orderInfo = orderInfos.get(0);
        ...
        if (data != null) {
            ...
            if (bundle != null) {
                if (bundle.getInt(XQRCode.RESULT_TYPE) == XQRCode.RESULT_SUCCESS) {
                    ...
                    int count = 0;
                    List<SortinginboundBarcode> barcodes = LitePal.where("barcode = ?", result).find(SortinginboundBarcode.class);
                    if (barcodes.size() > 0){
                        playSound(2);
                    ...
                    for (int i = 0; i < detailList2.size(); i++){
                        ...
                        if (pcigCode.equals(code)){
                            ...
                            SortinginboundBarcode salesBarcode = new SortinginboundBarcode(uuid, pcigCode, pcigName, result, scantime, scancode);
                            salesBarcode.save();
