package com.nti.module_sortinginbound.repository;
 ...
 * @describe
 */
public class SortinginboundRepo {
    public MutableLiveData<List<SortinginboundOrderInfo>> PDA_H(Paramer paramer){
        final MutableLiveData<List<SortinginboundOrderInfo>> data = new MutableLiveData<>();
        final List<SortinginboundOrderInfo> orderInfos = new ArrayList<>();
        HttpUtils.getInstance().with(IService.class).PDA_H(paramer)
                ...
                .subscribe(new Observer<JsonObject>() {
                    ...
                    public void onNext(@NotNull JsonObject jsonObject) {
                        ...
                        List<SortinginboundDetail> salesFactoryDetails = new ArrayList<>();
                        try {
                            for (int i = 0; i < jsonArray1.size(); i++){
                                ...
                                List<SortinginboundBarcode> barcodes = LitePal.where("UUID = ? and pcigcode = ? and isSubmit = 0", uuid, pickCode).find(SortinginboundBarcode.class);
                                ...
                                SortinginboundDetail salesFactoryDetail = new SortinginboundDetail(pickName, billPNum, billBNum, uuid, pickCode, scanBNum, String.valueOf(mscanQty));
                                salesFactoryDetails.add(salesFactoryDetail);
                            }
                            LitePal.deleteAll(ReturnoutboundBarcode.class);
                        ...
                        try {
                            ...
                            /*if (object.has("BB_TOTAL_SCAN_NUM")){
                                ...
                                List<SortinginboundDetail> details = LitePal.where("BD_BB_UUID = ?", BB_UUID).find(SortinginboundDetail.class);
                                ...
                                SortinginboundOrderInfo orderInfo = new SortinginboundOrderInfo(BB_UUID, BB_CONTRACT_NO, BB_BT_CODE, BB_TICKET_NO, BB_WS_CODE,
                                        BB_RELATE_CONTRACT_NO, B_NAME, BB_INPUT_DATE, BB_TOTAL_ALL_NUM1, BB_TOTAL_SCAN_NUM, BB_TOTAL_PNUM, BB_FLOW_NAME, BB_STATE);
                                orderInfos.add(orderInfo);
                            }
                            LitePal.deleteAll(SortinginboundOrderInfo.class);
