...
public class MoveinboundRepo {
    public MutableLiveData<List<MoveinboundOrderInfo>> PDA_H(Paramer paramer){
        final MutableLiveData<List<MoveinboundOrderInfo>> data = new MutableLiveData<>();
        ...
        HttpUtils.getInstance().with(IService.class).PDA_H(paramer)
                ...
                .subscribe(new Observer<JsonObject>() {
                    ...
                    public void onNext(@NotNull JsonObject jsonObject) {
                        ...
                        List<MoveinboundDetail> salesFactoryDetails = new ArrayList<>();
                        try {
                            for (int i = 0; i < jsonArray1.size(); i++){
                                ...
                                List<MoveinboundBarcode> barcodes = LitePal.where("UUID = ? and pcigcode = ? and isSubmit = 0", uuid, pickCode).find(MoveinboundBarcode.class);
                                ...
                                MoveinboundDetail salesFactoryDetail = new MoveinboundDetail(pickName, billPNum, billBNum, uuid, pickCode, scanBNum, String.valueOf(mscanQty));
                                salesFactoryDetails.add(salesFactoryDetail);
                            }
                            LitePal.deleteAll(MoveinboundDetail.class);
                        ...
                        try {
                            for (int i = 0; i < jsonArray.size(); i++){
                                ...
                                List<MoveinboundDetail> details = LitePal.where("BD_BB_UUID = ?", BB_UUID).find(MoveinboundDetail.class);
